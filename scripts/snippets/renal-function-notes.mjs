...
const FORMULA_BLOCKS = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      '**СКФ цистатин C (CKD-EPI eGFRcys 2021)**',
      'Уравнение CKD-EPI 2021 для eGFR по цистатину C:',
      'eGFR_cys = 133 × min(Scys/0,8, 1)^−0,499 × max(Scys/0,8, 1)^−1,328 × 0,996^возраст × [0,932 для женщин].',
      '• **Scys** — цистатин C, мг/л',
      '• **возраст** — в годах (≥ 18)',
      '• Если Scys ≤ 0,8, используется показатель −0,499; если Scys > 0,8 — показатель −1,328',
      '• Для мужчин множитель 0,932 не применяется',
