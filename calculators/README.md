# Калькуляторы

Каждый калькулятор — **отдельная папка**. При работе в Cursor откройте файл **только из своей папки** — правила не дают смотреть соседние калькуляторы; стиль единый через `shared/fc-calc.css`.

```
calculators/
  has-bled/
    index.html    ← единственный файл для сайта: <style> + HTML + <script>
    calc.js       ← формулы для тестов (на сайт не идёт)
    cases.json    ← эталонные кейсы
    meta.json     ← slug, title, source
```

Стили и скрипты в `index.html` **не должны влиять** на остальную страницу: всё внутри `<div class="fc-calc">`, селекторы только с префиксом `.fc-calc`.

На сайте ([калькуляторы](https://farmconsilium.com/calculator)) у всех виджетов **одинаковые зоны**: шапка → поля → кнопка → результат → дисклеймер. Каркас: `templates/LAYOUT.md`.

**Новые калькуляторы (эталон `hydro-balance`):**
- Шапка: `h2.fc-calc__title` + `p.fc-calc__hint`
- Поля в секциях: `fc-calc__panel-section` → `fc-calc__panel` → `h3.fc-calc__panel-heading`
- Стили панелей: `shared/fc-calc.css`; фрагмент: `templates/panel-section.html`

## Все Калькуляторы

Список генерируется из `meta.json` каждой папки. После создания или переименования калькулятора обновите `meta.json` → `title` и выполните:

```bash
node scripts/sync-calculator-readme.mjs
```

При `node scripts/new-calculator.mjs <slug>` список обновляется автоматически.

- **`abg-interpretation/`** — Интерпретация газов артериальной крови
- **`aims-scale/`** — Шкала AIMS
- **`aip/`** — Атерогенный индекс плазмы (AIP)
- **`aminophylline-peds/`** — Расчёт дозировки эуфиллина у детей до 16 лет
- **`anion-gap/`** — Калькулятор анионного разрыва
- **`anxiety-scales/`** — Шкалы тревоги
- **`apache-ii/`** — Шкала оценки APACHE II и оценка смертности
- **`apgar/`** — Шкала Апгар
- **`blood-gas/`** — Интерпретация кислотно-щелочного баланса и метаболических параметров по газам артериальной крови, электролитам и гемоглобину
- **`bmi/`** — Калькулятор расчёта индекса массы тела (BMI/ИМТ)
- **`bmr/`** — Калькулятор BMR
- **`bread-units/`** — Сколько хлебных единиц в рационе, калькулятор хлебных единиц (ХЕ)
- **`calcium-albumin/`** — Коррекция уровня кальция при гипоальбуминемии
- **`caprini-scale/`** — Шкала Caprini
- **`cardiac-output/`** — Калькулятор сердечного выброса
- **`cdai/`** — Индекс клинической активности заболевания ревматоидным артритом (CDAI)
- **`cha2ds2-va/`** — Шкала оценивает риск ишемического инсульта и системной тромбоэмболии у пациентов с фибрилляцией предсердий.
- **`child-pugh/`** — Чайлд-Пью и MELD
- **`ciwa-ar/`** — Шкала CIWA-AR
- **`date-diff/`** — Расчёт дней между двумя датами
- **`depression-scales/`** — Шкалы оценки депрессии
- **`dialysis-urea-reduction/`** — Процент снижения мочевины при гемодиализе
- **`dry-body-weight/`** — Калькулятор сухой вес тела
- **`dsi/`** — Диастолический шоковый индекс (DSI)
- **`eag-hba1c/`** — Средняя глюкоза (eAG) по HbA1c
- **`fena/`** — Фракционная экскреция натрия
- **`fluid-req/`** — Расчет физиологической потребности в жидкости
- **`gail-breast/`** — Модель Гейла — риск рака молочной железы (5 лет)
- **`ganzoni-iron/`** — Расчёт дозы внутривенного железа (формула Ганзони)
- **`gcs-equiv/`** — Клинический конвертер «Эквивалентные дозы глюкокортикостероидов (ГКС) — системные»
- **`geneva-pe/`** — Женевская шкала (индекс Geneva) оценки вероятности развития ТЭЛА
- **`gestation-due-date/`** — Определение срока беременности и даты родов
- **`glasgow-coma/`** — Шкала комы Глазго
- **`greene-climacteric/`** — Климактерическая шкала Грина
- **`hemodialysis-adequacy/`** — Определение адекватности гемодиализа
- **`homa-b/`** — Индекс HOMA-B
- **`homa-ir/`** — Индекс HOMA-IR
- **`hydro-balance/`** — Расчет гидробаланса
- **`hypertension-risk/`** — Степень и стадия артериальной гипертензии КП МЗ РБ № 38
- **`hyponatremia-infusion/`** — Коррекция нормы инфузии при гипонатриемии
- **`improve-scale/`** — Шкала IMPROVE
- **`infusomat/`** — Расчёт дозы и скорости введения препаратов на инфузомате (линеомате)
- **`insulin-tdd/`** — Калькулятор инсулина: суточная доза инсулина, коррекция гипергликемии и прандиальный болюс
- **`isotretinoin/`** — Калькулятор расчёта дозы изотретиноина для лечения акне
- **`lab-units/`** — Пересчёт единиц лабораторных показателей
- **`ldl-calc/`** — Расчёт ХС ЛНП
- **`ldl-lpa-corr/`** — Расчёт корригированного ХС ЛНП с учётом липопротеида(а)
- **`levothyroxine-dose/`** — Калькулятор дозы левотироксина на основе веса для гипотиреоза у взрослых
- **`lok-index/`** — Вероятность цирроза при гепатите C
- **`macocha/`** — Шкала MACOCHA
- **`martin-ldl/`** — Расчёт ХС ЛПНП
- **`mayo-coma/`** — Шкала комы FOUR
- **`mdq/`** — Скрининговый опросник для выявления расстройств биполярного спектра (MDQ)
- **`mental-status-scales/`** — Шкалы оценки психического статуса
- **`mkb10/`** — МКБ-10 — международная классификация болезней
- **`pars/`** — Оценка риска детской астмы (PARS)
- **`pasi/`** — Расчет индексов площади поражения и тяжести дерматологических заболеваний
- **`peds-percentiles/`** — Оценка физического развития детей
- **`pesi-pe/`** — Шкала PESI оценки летальности пациентов с ТЭЛА
- **`potassium-deficit/`** — Дефицит калия
- **`precise-dapt/`** — Шкала PRECISE-DAPT
- **`pregnancy-dating/`** — Определение срока беременности по последней менструации и ультразвуковой биометрии
- **`quicki/`** — Калькулятор QUICKI
- **`renal-function/`** — Калькулятор оценки функции почек: CKD-EPIcr 2021, CKD-EPIcr-cys 2021, eGFR(BSAadj), Cockcroft-Gault, KDIGO-матрица риска, BSA, BMI
- **`score2/`** — SCORE2 / SCORE2-OP — оценка сердечно-сосудистого риска
- **`serum-osmolality/`** — Расчёт осмоляльности сыворотки
- **`sodium-deficit/`** — Дефицит натрия при гипонатриемии
- **`sodium-hyperglycemia/`** — Коррекция натрия при гипергликемии
- **`sofa/`** — Динамическая оценка органной недостаточности SOFA / qSOFA
- **`stop-bang/`** — Шкала STOP-BANG для оценки риска обструктивного апноэ сна
- **`tela-scales/`** — Шкалы ТЭЛА и ВТЭО
- **`tyg-index/`** — Индекс TyG
- **`urea-distribution-volume/`** — Расчёт объёма распределения мочевины
- **`wells-pe/`** — Шкала Уэллса для оценки вероятности ТЭЛА
- **`wells-scale/`** — Шкала Веллса
- **`years-pe/`** — Алгоритм YEARS при подозрении на ТЭЛА

## Новый калькулятор

```bash
node scripts/new-calculator.mjs has-bled
```

После заполнения `meta.json` (название) пересоберите список, если меняли title вручную:

```bash
node scripts/sync-calculator-readme.mjs
```

## Обновить стили во всех / одном

Общий файл: `shared/fc-calc.css`. После изменения:

```bash
node scripts/sync-calculator-styles.mjs has-bled
```

## Проверка

```bash
node --test tests/example.test.mjs
```

Правила Cursor: `calculator-scope.mdc`, `calculator-style.mdc`.  
Дизайн-токены: `FRONTEND-STYLES.md` §13–16.  
**На сайт:** весь `index.html` в админку — см. `PUBLISH.md`.
